...
#error Unknown architecture.
            ...
            F &&f)
        {
            ...
            return {n, static_cast<std::size_t>(std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count()), 0};
        }
            ...
            for (unsigned i = 0; i < vf.size(); ++i)
            {
                for (unsigned j = 0; j < vi.size(); ++j)
                {
                    ...
                    for (unsigned k = 0; k < Trials; ++k)
                    {
                        ...
                        dout << verb << " " << vf[i].name << "," << toolset << " " << arch << "," << vi[j]->name() << "," << result.calls << "," << result.millis << "," << result.mbs << "\n";
                    ...
                    auto const mbs = static_cast<
                        std::size_t>((0.5 + 1000.0 *
                                                ...
                                                millis / 1024 / 1024));
                    strout << verb << " " << vf[i].name << "," << toolset << " " << arch << "," << vi[j]->name() << "," << mbs << "\n";
                }
            }
        }
    ...
    char const *const *const argv)
{
    if (argc < 2)
    {
        std::cerr << "Usage: bench [options...] <file>...\n"
                     "\n"
