...
/** Return parsed JSON as a @ref value.
    ...
    This function parses input in one step to produce a complete JSON
    @ref value. If the input does not contain a complete serialized JSON, an
    error occurs. In this case (1), (2), (4), and (5) return a null value that
    uses the [default memory resource], and set `ec` to the corresponding error
    value. (3) and (6) throw an exception.
    ...
    (3), (6) throw `boost::system::system_error` on failed parse.
    Calls to `memory_resource::allocate` may throw.
    The stream `is` may throw as described by [`std::ios::exceptions`].
    ...
    @param opt The options for the parser. If this parameter is omitted,
        the parser will accept only standard JSON.

    @throw boost::system::system_error Thrown on failure: (3), (6).
