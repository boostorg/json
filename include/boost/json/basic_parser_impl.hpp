...
#include <boost/json/detail/fast_double_parser.h>
...
{
    BOOST_ASSERT(exp >= 0 && exp < 309);
    return fast_double_parser::power_of_ten[exp];
}
...
dec_to_float(
    ...
    bool neg) noexcept
{
    if(BOOST_JSON_LIKELY((e >= FASTFLOAT_SMALLEST_POWER) &&
            (e <= FASTFLOAT_LARGEST_POWER))) {
        bool success = true;
        double r = fast_double_parser::compute_float_64(e, m, neg, &success);
        if(BOOST_JSON_LIKELY(success))
        {
            return r;
        }
    }
    ...
    return e < 0 ? x / pow10(-e) : x * pow10(e);
}
