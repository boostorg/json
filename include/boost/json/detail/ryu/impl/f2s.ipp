...
#ifndef BOOST_JSON_DETAIL_RYU_IMPL_F2S_IPP
...
constexpr int FLOAT_EXPONENT_BITS = 8;
constexpr int FLOAT_BIAS = 127;

// This table is generated by PrintFloatLookupTable.
constexpr int FLOAT_POW5_INV_BITCOUNT = 59;

inline
std::uint64_t const
(&FLOAT_POW5_INV_SPLIT() noexcept)[31]
{
    static constexpr std::uint64_t arr[31] = {
...
constexpr int FLOAT_POW5_BITCOUNT = 61;

inline
std::uint64_t const
(&FLOAT_POW5_SPLIT() noexcept)[47]
{
    static constexpr std::uint64_t arr[47] = {
    ...
    const std::int32_t shift)
{
  ...
  // function.
  const std::uint32_t factorLo = (std::uint32_t)(factor);
  const std::uint32_t factorHi = (std::uint32_t)(factor >> 32);
  const std::uint64_t bits0 = (std::uint64_t)m * factorLo;
  const std::uint64_t bits1 = (std::uint64_t)m * factorHi;

#ifdef BOOST_JSON_DETAIL_RYU_32_BIT_PLATFORM
    // On 32-bit platforms we can avoid a 64-bit shift-right since we only
    // need the upper 32 bits of the result and the shift value is > 32.
    const std::uint32_t bits0Hi = (std::uint32_t)(bits0 >> 32);
    std::uint32_t bits1Lo = (std::uint32_t)(bits1);
    std::uint32_t bits1Hi = (std::uint32_t)(bits1 >> 32);
    bits1Lo += bits0Hi;
...
floating_decimal_32
f2d(
    const std::uint32_t ieeeMantissa,
    const std::uint32_t ieeeExponent)
{
    ...
    {
        e2 = (std::int32_t)ieeeExponent - FLOAT_BIAS - FLOAT_MANTISSA_BITS - 2;
    ...
    if (e2 >= 0)
    {
        const std:: uint32_t q = log10Pow2(e2);
        e10 = (std::int32_t)q;
        const std::int32_t k = FLOAT_POW5_INV_BITCOUNT + pow5bits((std::int32_t)q) - 1;
        ...
        if (q != 0 && (vp - 1) / 10 <= vm / 10)
        {
            ...
            const std::int32_t l = FLOAT_POW5_INV_BITCOUNT + pow5bits((std::int32_t)(q - 1)) - 1;
            lastRemovedDigit = (std::uint8_t)(mulPow5InvDivPow2(mv, q - 1, -e2 + (std::int32_t)q - 1 + l) % 10);
        }
        if (q <= 9)
        {
    ...
    {
        const std::uint32_t q = log10Pow5(-e2);
        e10 = (std::int32_t)q + e2;
        const std::int32_t i = -e2 - (std::int32_t)q;
        const std::int32_t k = pow5bits(i) - FLOAT_POW5_BITCOUNT;
        int32_t j = (int32_t)q - k;
        vr = mulPow5divPow2(mv, (std::uint32_t)i, j);
        vp = mulPow5divPow2(mp, (std::uint32_t)i, j);
        vm = mulPow5divPow2(mm, (std::uint32_t)i, j);
#ifdef RYU_DEBUG
        printf("%u * 5^%d / 10^%u\n", mv, -e2, q);
        printf("%u %d %d %d\n", q, i, k, j);
        printf("V+=%u\nV =%u\nV-=%u\n", vp, vr, vm);
        ...
        if (q != 0 && (vp - 1) / 10 <= vm / 10)
        {
            ...
            lastRemovedDigit = (std::uint8_t)(mulPow5divPow2(mv, (std::uint32_t)(i + 1), j) % 10);
        }
        if (q <= 1) {
    ...
    char* const result)
{
    ...
    std::uint32_t output = v.mantissa;
    const std::uint32_t olength = decimalLength9(output);
    ...
    while (output >= 10000)
    {
        ...
        const std::uint32_t c = output - 10000 * (output / 10000);
#else
        const std::uint32_t c = output % 10000;
#endif
        output /= 10000;
    ...
    // Print the exponent.
    result[index++] = 'E';
    std::int32_t exp = v.exponent + (std::int32_t)olength - 1;
    if (exp < 0)
    {
        result[index++] = '-';
        exp = -exp;
    ...
    char* result) noexcept
{
    ...
    const std::uint32_t bits = float_to_bits(f);
    ...
    const std::uint32_t ieeeMantissa = bits & ((1u << FLOAT_MANTISSA_BITS) - 1);
    const std::uint32_t ieeeExponent = (bits >> FLOAT_MANTISSA_BITS) & ((1u << FLOAT_EXPONENT_BITS) - 1);
...
char*
f2s(float f) noexcept
{
    static thread_local char result[16];
